...
function(input, output, session) {
  ...
  observe({
    ...
    if (sizeBy == "liveable_h") {
      ...
      radius <- ifelse(zipdata$centile >= (100 - input$threshold), 2000, 200)
    } else {
      radius <- zipdata[[sizeBy]] / max(zipdata[[sizeBy]]) * 1500
    }
  ...
  # When map is clicked, show a popup with suburb info
  observe({
    ...
    postcodes <- if (is.null(input$states)) character(0) else {
      cleantable %>%
        filter(State %in% input$states,
          is.null(input$cities) | suburb %in% input$cities) %>%
        `$`('postcode') %>%
        unique() %>%
        sort()
    }
